...
class mysql{
    ...
    function connect($config){
        extract($config);
        if(!(self::$con = mysqli_connect($dbhost,$dbuser,$dbpsw, $dbname))){//mysql_connect连接数据库函数
            $this->err(mysqli_error());
        }
        /*if(!mysqli_select_db($dbname,$con)){//mysql_select_db选择库的函数
            $this->err(mysqli_error());
        }*/
        mysqli_query(self::$con,"set names ".$dbcharset);//使用mysql_query 设置编码  格式：mysql_query("set names utf8")
    }
    /**
     * 执行sql语句
     *
     ...
     */
    function query($sql){
        if(!($query = mysqli_query(self::$con, $sql))){//使用mysql_query函数执行sql语句
